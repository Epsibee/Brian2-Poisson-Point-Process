...
def plot_stats(theoretical_means, empirical_mean, theoretical_std_dev, empirical_std_dev, title = 'title', show_hline=False, data_type='standardized'):
    """
    ...
    if show_hline:
        ...
        elif data_type == 'centered':
            # sqrt_values = np.sqrt(theory_std_pad)
            # For centered data: mean=0, but std is from original data
            
            # rate = final variance at time t / t ...aka std^2 / t
            rate = (theory_std_pad[-1] ** 2) / (len(time_points) - 1)
    
            # var(t) = rate x t
            # std = sqrt(rate × t)
            theoretical_std_curve = np.sqrt(rate * time_points)
            theoretical_std_curve[0] = 0  # Avoid issues at t=0

            ax.plot(time_points, -theoretical_std_curve, color='black', linestyle='--', linewidth=1.5)
            ax.plot(time_points, theoretical_std_curve, color='black', linestyle='--', linewidth=1.5)
...
def plot_multiple_stats(theoretical_means, empirical_mean, theoretical_std_dev, empirical_std_dev, title='title', show_hline=False, data_type='standardized'):
    """
...
def multivariate_log_likelihood(data, mean = np.nan, cov = np.nan):
    """
    ...
    if mean is np.nan:
        mean_vec = np.zeros(num_neurons)
        cov_matrix = np.eye(num_neurons)
    ...
    log_likelihood_per_sample = multivariate_normal.logpdf(data.T, mean=mean_vec, cov=cov_matrix)
    
    # Total log-likelihood
    total_log_likelihood = np.sum(log_likelihood_per_sample)
    average_log_likelihood = np.sum(log_likelihood_per_sample) / num_neurons
    
    return total_log_likelihood, log_likelihood_per_sample, average_log_likelihood


# In[49]:


def chi_square_test(observed, expected, n_bins=50, min_expected=5):
    """
    chi-square test by binning continuous data with safety checks
    """
    # Flatten data
...
def match_array_shapes(array1, array2):
    """
    Make two arrays the same shape by trimming to the smaller size along each dimension.
    Works for arrays of any dimensionality (1D, 2D, 3D, etc.)
    
    Args:
        array1: First numpy array (any shape)
        array2: Second numpy array (any shape, must have same number of dimensions as array1)
    ...
    # Check if arrays have the same number of dimensions
    if array1.ndim != array2.ndim:
        raise ValueError(f"Arrays must have same number of dimensions. "
                        f"Got {array1.ndim}D and {array2.ndim}D")
    
    # Build slicing tuples for both arrays
    slices1 = []
    slices2 = []
    
    # For each dimension, find the minimum size and create slice
    for dim_size1, dim_size2 in zip(array1.shape, array2.shape):
        min_size = min(dim_size1, dim_size2)
        slices1.append(slice(0, min_size))
        slices2.append(slice(0, min_size))
    
    # Apply slicing
    array1_resized = array1[tuple(slices1)]
    array2_resized = array2[tuple(slices2)]
...
def two_tailed_z_pvalue(z_stat):
    """
    Calculate two-tailed p-value using error function.
    Formula: p = 2 * (1 - erf(|Z| / sqrt(2))) = erfc(|Z| / sqrt(2))
    """
    return 2 * (1 - erf(np.abs(z_stat) / np.sqrt(2)))


from scipy.stats import norm
import numpy as np

def cumulative_z_test(data, population_mean=0):
    """
    Data = standardized with the null model empirical data flattened 
    We may use the theoretical or empirical null model.
    Compute Z-statistics as observations develop over time.
    data: 1D array of observations
    population_mean: expected mean under null hypothesis

    Describe why the population mean is zero, we are dividing by 1 = aka std dev
    """
    n_obs = len(data)
    n_array = np.arange(1, n_obs + 1)

    ## Z = (estimated mean - population_mean) / np.sqrt(n)
    ## Z = (T - mu) / s where , sum of Z_i, s = sigma^2/n
    # 1/sqrt
    cumulative_z_stats = (np.cumsum(data)/ np.sqrt(n_array) - population_mean*np.sqrt(n_array)) # /1
    ## p = 2 * (1 - Φ(|z|)) , where Z ~ N(0,1) and Φ is the standard normal CDF ##
    # p = 2 × [1 - Φ(|z|)] = erfc(|z| / sqrt(2)) # 
    cumulative_p_values = 2 * (norm.cdf(-np.abs(cumulative_z_stats)))
    # cumulative_p_values = erfc(np.abs(cumulative_z_stats) / np.sqrt(2))
    return cumulative_z_stats, cumulative_p_values


import matplotlib.pyplot as plt
import numpy as np

def plot_cumulative_analysis(data_flat, z_stats_baseline, p_values_baseline,
                            data_flat_pert=None, z_stats_pert=None, p_values_pert=None,
                            title='Cumulative Analysis of Standardized Data',
                            baseline_label='Baseline Model',
                            perturbed_label='Perturbed Model',
                            figsize=(12, 14), save_path=None,
                            threshold=1e-4):
    """
    Create a comprehensive 4-panel plot for cumulative statistical analysis.
    
    Parameters:
    -----------
    data_flat : array-like
        Standardized data for baseline model
    z_stats_baseline : array-like
        Cumulative Z-statistics for baseline model
    p_values_baseline : array-like
        Cumulative p-values for baseline model
    data_flat_pert : array-like, optional
        Standardized data for perturbed model
    z_stats_pert : array-like, optional
        Cumulative Z-statistics for perturbed model
    p_values_pert : array-like, optional
        Cumulative p-values for perturbed model
    title : str, optional
        Main title for the figure
    baseline_label : str, optional
        Label for baseline model in legend
    perturbed_label : str, optional
        Label for perturbed model in legend
    figsize : tuple, optional
        Figure size (width, height)
    save_path : str, optional
        Path to save the figure. If None, figure is not saved.
    threshold : float, optional
        P-value threshold to mark (default: 1e-4)
    
    Returns:
    --------
    fig, axes : matplotlib figure and axes objects
    threshold_info : dict
        Dictionary containing threshold crossing information
    """
    ...
    # Time axis (cumulative samples)
    time_axis = np.arange(1, len(data_flat) + 1)
    
    # Dictionary to store threshold crossing information
    threshold_info = {}
    
    # Define alpha levels to track
    alpha_levels = [0.05, 0.01, 0.001, 0.0001]
    alpha_colors = ['red', 'darkred', 'maroon', 'purple']
    
    # ============================================
    # Plot 1: Raw Data
    # ============================================
    axes[0].plot(time_axis, data_flat, linewidth=0.8, alpha=0.7, 
                label=baseline_label, color='steelblue')
    if data_flat_pert is not None:
        axes[0].plot(time_axis, data_flat_pert, linewidth=0.8, alpha=0.7,
                    label=perturbed_label, color='coral')
    axes[0].axhline(y=0, color='black', linestyle='--', linewidth=1, alpha=0.5)
    axes[0].set_ylabel('Standardized Data', fontsize=12, fontweight='bold')
    axes[0].set_xlabel('Cumulative Sample Number', fontsize=12, fontweight='bold')
    axes[0].set_title('(A) Standardized Spike Counts', fontsize=12, loc='left')
    ...
    # Plot 2: Z-Statistics
    # ============================================
    axes[1].plot(time_axis, z_stats_baseline, linewidth=1.2, 
                label=baseline_label, color='steelblue')
    if z_stats_pert is not None:
        axes[1].plot(time_axis, z_stats_pert, linewidth=1.2,
                    label=perturbed_label, color='coral')
    axes[1].axhline(y=0, color='black', linestyle='--', linewidth=1, alpha=0.5)
    axes[1].axhline(y=1.96, color='red', linestyle=':', linewidth=1.5, 
                   alpha=0.7, label='p=0.05 threshold')
    axes[1].axhline(y=-1.96, color='red', linestyle=':', linewidth=1.5, alpha=0.7)
    axes[1].set_ylabel('Z-Statistic', fontsize=12, fontweight='bold')
    axes[1].set_xlabel('Cumulative Sample Number', fontsize=12, fontweight='bold')
    axes[1].set_title('(B) Cumulative Z-Statistics', fontsize=12, loc='left')
    ...
    # Plot 3: P-Values
    # ============================================
    axes[2].plot(time_axis, p_values_baseline, linewidth=1.2, 
                label=baseline_label, color='steelblue')
    if p_values_pert is not None:
        axes[2].plot(time_axis, p_values_pert, linewidth=1.2,
                    label=perturbed_label, color='coral')
    axes[2].axhline(y=0.05, color='red', linestyle='--', linewidth=1.5, 
                   alpha=0.7, label='α=0.05')
    axes[2].axhline(y=0.01, color='darkred', linestyle='--', linewidth=1.5, 
                   alpha=0.7, label='α=0.01')
    axes[2].set_ylabel('P-Value', fontsize=12, fontweight='bold')
    axes[2].set_xlabel('Cumulative Sample Number', fontsize=12, fontweight='bold')
    axes[2].set_title('(C) Cumulative P-Values', fontsize=12, loc='left')
    axes[2].set_ylim([0, 1])
    axes[2].legend(loc='best', framealpha=0.9)
    axes[2].grid(True, alpha=0.3)
    
    # ============================================
    # Plot 4: Log P-Values with Alpha Crossings
    # ============================================
    # Add small epsilon to avoid log(0)
    epsilon = 1e-300
    log_p_baseline = np.log10(p_values_baseline + epsilon)
    
    axes[3].plot(time_axis, log_p_baseline, linewidth=1.2, 
                label=baseline_label, color='steelblue')
    
    # Find threshold crossing for baseline
    threshold_idx_baseline = np.where(p_values_baseline < threshold)[0]
    if len(threshold_idx_baseline) > 0:
        first_cross_baseline = threshold_idx_baseline[0]
        threshold_info['baseline_sample'] = first_cross_baseline + 1
        threshold_info['baseline_p_value'] = p_values_baseline[first_cross_baseline]
    
    if p_values_pert is not None:
        log_p_pert = np.log10(p_values_pert + epsilon)
        axes[3].plot(time_axis, log_p_pert, linewidth=1.2,
                    label=perturbed_label, color='coral')
        
        # Find threshold crossing for perturbed at main threshold
        threshold_idx_pert = np.where(p_values_pert < threshold)[0]
        if len(threshold_idx_pert) > 0:
            first_cross_pert = threshold_idx_pert[0]
            threshold_info['perturbed_sample'] = first_cross_pert + 1
            threshold_info['perturbed_p_value'] = p_values_pert[first_cross_pert]
        
        # Find and plot crossings for each alpha level (PERTURBED MODEL ONLY)
        threshold_info['alpha_crossings'] = {}
        for alpha, color in zip(alpha_levels, alpha_colors):
            crossing_idx = np.where(p_values_pert < alpha)[0]
            if len(crossing_idx) > 0:
                first_crossing = crossing_idx[0]
                crossing_sample = first_crossing + 1
                crossing_p_value = p_values_pert[first_crossing]
                
                # Store info
                threshold_info['alpha_crossings'][alpha] = {
                    'sample': crossing_sample,
                    'p_value': crossing_p_value
                }
                
                # Add vertical line
                axes[3].axvline(x=crossing_sample, color=color, 
                               linestyle='-.', linewidth=1.5, alpha=0.6,
                               label=f'Pert. crosses α={alpha} at n={crossing_sample}')
    
    # Add horizontal threshold lines
    axes[3].axhline(y=np.log10(0.05), color='red', linestyle='--', 
                   linewidth=1.5, alpha=0.7, label='α=0.05')
    axes[3].axhline(y=np.log10(0.01), color='darkred', linestyle='--', 
                   linewidth=1.5, alpha=0.7, label='α=0.01')
    axes[3].axhline(y=np.log10(0.001), color='maroon', linestyle='--', 
                   linewidth=1.5, alpha=0.7, label='α=0.001')
    axes[3].axhline(y=np.log10(0.0001), color='purple', linestyle='--', 
                   linewidth=1.5, alpha=0.7, label='α=0.0001')
    
    axes[3].set_ylabel('Log₁₀(P-Value)', fontsize=12, fontweight='bold')
    axes[3].set_xlabel('Time Step', fontsize=12, fontweight='bold')
    axes[3].set_title('(D) Log-Scale P-Values', fontsize=12, loc='left')
    axes[3].legend(loc='best', framealpha=0.9, fontsize=8, ncol=2)
    ...
    print(f"\n{'='*60}")
    print(f"Threshold Crossing Analysis:")
    print(f"{'='*60}")
    
    if 'baseline_sample' in threshold_info:
        print(f"\n{baseline_label}:")
        print(f"  First crosses p < {threshold} at sample: {threshold_info['baseline_sample']}")
        print(f"  P-value at crossing: {threshold_info['baseline_p_value']:.2e}")
    
    if 'alpha_crossings' in threshold_info and threshold_info['alpha_crossings']:
        print(f"\n{perturbed_label} - Alpha Level Crossings:")
        print(f"  {'Alpha':<10} {'Sample':<10} {'P-value'}")
        print(f"  {'-'*40}")
        for alpha in alpha_levels:
            if alpha in threshold_info['alpha_crossings']:
                info = threshold_info['alpha_crossings'][alpha]
                print(f"  {alpha:<10} {info['sample']:<10} {info['p_value']:.2e}")
    
    if 'baseline_sample' in threshold_info and 'perturbed_sample' in threshold_info:
        diff = threshold_info['perturbed_sample'] - threshold_info['baseline_sample']
        print(f"\nDifference at p < {threshold}: {diff} samples")
    
    print(f"{'='*60}\n")
    
    return fig, axes, threshold_info


import numpy as np
import matplotlib.pyplot as plt

def count_and_raster_plot(count, num_samples, title='Count and Raster Plot', theoretical_mean=None):
    """
    Single-axes plot showing:
    - Counting process trajectories (step plots) for Neuron 1 across samples
    - Raster ticks (one row per sample) near the bottom of the same axes
    - Empirical mean across the first `num_samples` (red)
    - Optional theoretical mean (blue), if provided

    Parameters
    ----------
    count : list-like
        count[i][0] is a 1D array for sample i giving the cumulative count over time (Neuron 1).
    num_samples : int
        Number of samples to include (from the start of `count`).
    title : str
        Plot title.
    theoretical_mean : array-like or None
        Optional theoretical mean curve (same length as the time series). If provided, it is plotted in blue.
    """
    S = min(num_samples, len(count))
    if S == 0:
        raise ValueError("No samples to plot.")

    # Use the shortest trajectory length to ensure alignment
    lengths = [len(np.asarray(count[i][0])) for i in range(S)]
    T = min(lengths)
    t = np.arange(T)  # assume 1 ms per index

    # Stack the selected counting processes (trimmed to T)
    trajectories = np.stack([np.asarray(count[i][0])[:T] for i in range(S)], axis=0)
    ...
    # Plot counting process trajectories (step plots)
    for i in range(S):
        plt.step(t, trajectories[i], where='post', color=colors[i], alpha=0.55, linewidth=1.5)

    # Means
    emp_mean = trajectories.mean(axis=0)
    theory_line = None
    emp_line = None

    if theoretical_mean is not None:
        theory = np.asarray(theoretical_mean)[:T]
        theory_line, = plt.plot(t, theory, color='tab:blue', linewidth=2.5, label='Theoretical Mean')

    emp_line, = plt.plot(t, emp_mean, color='tab:red', linewidth=2.5, label='Empirical Mean')

    # Raster ticks near the bottom of the same axes
    tick_height = 0.35  # height of each tick
    base_offset = -tick_height * (S + 0.6)  # bottom offset to fit all raster rows
    for i in range(S):
        c = trajectories[i]
        diffs = np.diff(c, prepend=0)     # treat positive increments as spikes
        spike_idx = np.where(diffs > 0)[0]
        reps = diffs[spike_idx].astype(int)
        x_spikes = np.repeat(spike_idx, reps)

        y0 = base_offset + tick_height * (i + 0.2)  # vertical row position for sample i
        if x_spikes.size > 0:
            plt.vlines(x_spikes, y0, y0 + tick_height * 0.8, color=colors[i], lw=0.9)

    # Labels, limits, and styling
    ymax = float(np.nanmax(trajectories)) if trajectories.size else 1.0
    plt.xlabel('Time (ms)')
    plt.ylabel('Count of Neuron 1')
    plt.title(title)
    plt.ylim(base_offset - tick_height * 0.5, max(ymax * 1.05, np.nanmax(emp_mean) * 1.05))
    plt.grid(True, alpha=0.25)

    # Legend (ensure order: Theoretical (blue), Empirical (red))
    handles = []
    if theory_line is not None:
        handles.append(theory_line)
    if emp_line is not None:
        handles.append(emp_line)
    if handles:
